...
struct MaceContext {
  ...
  std::string model_data_name;
  FileMapper *model_fm = NULL;
  FileMapper *model_data_fm = NULL;
  ...
  std::map<std::string, ModelInfo> model_infos = {
      ...
      {"mobilenet_v2_quant", {"input", "output",
                                  {1, 224, 224, 3}, {1, 1001}}},
      // DeepAction Models
      {"/storage/emulated/0/mace_workspace/models/tf_resnet152.pb",
          {"input", "dense/BiasAdd", {1, 224, 224, 3}, {1, 101}}},
      {"/storage/emulated/0/mace_workspace/models/tf_resnet18_mv.pb",
...
Java_com_xiaomi_mace_JniMaceUtils_maceCreateGPUContext(
...
Java_com_xiaomi_mace_JniMaceUtils_maceCreateEngine(
  ...
  // Parse model data name
  const char *model_data_name_ptr = env->GetStringUTFChars(model_data_name_str, nullptr);
  if (model_data_name_ptr == nullptr) return JNI_ERR;
  mace_context.model_data_name.assign(model_data_name_ptr);
  env->ReleaseStringUTFChars(model_data_name_str, model_data_name_ptr);
...
*/
  ...
  std::unique_ptr<mace::port::ReadOnlyMemoryRegion> model_weights_data =
    make_unique<mace::port::ReadOnlyBufferMemoryRegion>();
  auto fs = GetFileSystem();
  status = fs->NewReadOnlyMemoryRegionFromFile(mace_context.model_data_name.c_str(),
                                               &model_weights_data);
  if (status != MaceStatus::MACE_SUCCESS) {
    __android_log_print(ANDROID_LOG_ERROR,
                        "image_classify",
  ...
  *****/
  ...
  //__android_log_print(ANDROID_LOG_INFO, "image_classify", "[BENGIN] Map model file");
  
  // Load model graph and weights data with FileMapper
  mace_context.model_fm      = createReadOnlyFileMapper(mace_context.model_name.c_str());
  mace_context.model_data_fm = createReadOnlyFileMapper(mace_context.model_data_name.c_str());
  if (!mace_context.model_fm || !mace_context.model_data_fm) {
      __android_log_print(ANDROID_LOG_ERROR,
                        "image_classify",
                        "Map file result is NULL");
      return JNI_ERR;
  }
  
  //__android_log_print(ANDROID_LOG_INFO, "image_classify", "[END] Map model file");
  
  // Create MaceEngine instance from model file
  mace::MaceStatus create_engine_status;
  
  create_engine_status =
    CreateMaceEngineFromProto(reinterpret_cast<const unsigned char *>(mace_context.model_fm->data),
                              mace_context.model_fm->size,
                              reinterpret_cast<const unsigned char *>(mace_context.model_data_fm->data),
                              mace_context.model_data_fm->size,
                              input_names,
                              output_names,
                              config,
                              &mace_context.engine);
